...
const emitDefinitelyUsed = (file: SourceFile, onResult: OnResultType) => {
  [...importWildCards(file), ...exportWildCards(file)].forEach(onResult);
};
...
const emitPotentiallyUnused = (file: SourceFile, onResult: OnResultType) => {
  ...
  const referenceCounts = countBy(
    identity,
    (idsInFile || []).map((node) => node.getText())
  );
  const referencedInFile = Object.entries(
    referenceCounts
  ).flatMap(([name, count]) => (count > 1 ? [name] : []));
